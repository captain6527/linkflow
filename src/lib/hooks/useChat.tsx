...
const checkConfig = async (
  ...
  try {
    ...
    setChatModelProvider({ key: chatModelKey, providerId: chatModelProviderId });
    setEmbeddingModelProvider({ key: embeddingModelKey, providerId: embeddingModelProviderId });
...
const loadMessages = async (
  ...
  messages.forEach((msg) => {
    ...
    // NOTE: history only stores the assistant "answer" text, NOT thinking
    const textBlocks = msg.responseBlocks
      .filter((block): block is Block & { type: 'text' } => block.type === 'text')
      .map((block) => block.data)
      .join('\n');

    if (textBlocks) history.push(['assistant', textBlocks]);
  });
...
// ✅ 新增：通用 NDJSON 读取器（只解析完整行，保留半行）
async function consumeNdjsonStream(
  reader: ReadableStreamDefaultReader<Uint8Array>,
  decoder: TextDecoder,
  onJson: (obj: any) => void | Promise<void>,
) {
  let partial = '';
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // 这一行如果解析失败，说明它很可能不是完整 JSON（或后端输出污染）
        // 拼回 partial，等待下一 chunk 补齐
        partial = s + '\n' + partial;
        break;
      }
    }
  ...
  // done 后，如果还残留一行，尝试最后 parse 一次（可选）
...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const sections = useMemo<Section[]>(() => {
    // 兜底：如果 text 里还夹了 <think>...</think>，剥离掉，保证 Answer 纯净
    const stripThinkXml = (s: string) => {
      if (!s) return s;
    ...
    return messages.map((msg) => {
      ...
      let speechMessage = '';
      const thinkingEnded = msg.status !== 'answering';
      let suggestions: string[] = [];

      const sourceBlocks = msg.responseBlocks.filter(
        (block): block is Block & { type: 'source' } => block.type === 'source',
      );
      const srcs = sourceBlocks.flatMap((block) => block.data);

      const widgetBlocks = msg.responseBlocks
        .filter((b) => b.type === 'widget')
        .map((b) => b.data) as Widget[];

      // ✅ thinking blocks 单独收集
      const thinkingBlocks = msg.responseBlocks.filter(
        (block): block is Block & { type: 'thinking' } => block.type === 'thinking',
      );
      const thinkingText = thinkingBlocks.map((b) => b.data).join('\n\n');

      msg.responseBlocks.forEach((block) => {
        if (block.type === 'text') {
          let processedText = stripThinkXml(block.data);
              ...
              (_, capturedContent: string) => {
                ...
                const linksHtml = numbers
                  .map((numStr) => {
                    ...
                    const source = srcs[number - 1];
                    const url = source?.metadata?.url;
                    return url ? `<citation href="${url}">${numStr}</citation>` : ``;
                  })
                  .join('');
          ...
          if (processedText.trim()) textBlocks.push(processedText);
  ...
  const checkReconnect = async () => {
    ...
    if (messages.length > 0) {
      ...
      if (lastMsg.status === 'answering') {
        ...
        try {
          // ✅ FIX: 只解析完整行 JSON
          await consumeNdjsonStream(reader, decoder, async (json) => {
  ...
  useEffect(() => {
    if (chatId && !newChatCreated && !isMessagesLoaded && messages.length === 0) {
      loadMessages(chatId, setMessages, setIsMessagesLoaded, chatHistory, setSources, setNotFound, setFiles, setFileIds);
    } else if (!chatId) {
  ...
  const getMessageHandler = (message: Message) => {
    ...
    return async (data: any) => {
      if (data.type === 'error') {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.messageId === messageId ? { ...msg, status: 'error' as const } : msg,
          ),
        );
        return;
      }
      ...
      if (data.type === 'researchComplete') {
        ...
        if (message.responseBlocks.find((b) => b.type === 'source' && b.data.length > 0)) {
        ...
        setMessages((prev) =>
          prev.map((msg) => {
            if (msg.messageId === messageId) {
              const exists = msg.responseBlocks.findIndex((b) => b.id === data.block.id);
              ...
              return { ...msg, responseBlocks: [...msg.responseBlocks, data.block] };
            }
            return msg;
          }),
        );
      ...
      if (data.type === 'updateBlock') {
            ...
            if (msg.messageId === messageId) {
              const updatedBlocks = msg.responseBlocks.map((block) => {
                if (block.id === data.blockId) {
                  ...
                  if (updatedBlock.type === 'text' || updatedBlock.type === 'thinking') {
                    shouldAppear = true;
                  }
        ...
        if (shouldAppear) setMessageAppeared(true);
      }

      if (data.type === 'messageEnd') {
        if (handledMessageEndRef.current.has(messageId)) return;
        handledMessageEndRef.current.add(messageId);

        const currentMsg = messagesRef.current.find((msg) => msg.messageId === messageId);
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.messageId === messageId ? { ...msg, status: 'completed' as const } : msg,
          ),
        );
        ...
        if (hasSourceBlocks && !hasSuggestions) {
          ...
          setMessages((prev) =>
            prev.map((msg) => {
              if (msg.messageId === messageId) {
                return { ...msg, responseBlocks: [...msg.responseBlocks, suggestionBlock] };
              }
              return msg;
            }),
          );
  ...
  const sendMessage: ChatContext['sendMessage'] = async (message, messageId, rewrite = false) => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        content: message,
        message: { messageId: messageId, chatId: chatId!, content: message },
        chatId: chatId!,
        files: fileIds,
        sources: sources,
        optimizationMode: optimizationMode,
        history: rewrite
          ? chatHistory.current.slice(0, messageIndex === -1 ? undefined : messageIndex)
          : chatHistory.current,
        chatModel: { key: chatModelProvider.key, providerId: chatModelProvider.providerId },
        embeddingModel: { key: embeddingModelProvider.key, providerId: embeddingModelProvider.providerId },
